...
- Prefer describing patterns over copying pixels. PptSmith must still output registered layouts, components, and editable PPTX objects.
...
- The style brief maps to PptSmith theme, template, `themeOverrides`, preferred layouts, and fallback risks.
- Protected assets and source-specific content are listed under `doNotReuse` unless user authorization is explicit.
- The resulting deck remains a PptSmith presentation, not a website clone or image replica.
