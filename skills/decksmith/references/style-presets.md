...
2. If a style reference exists, read `.decksmith/source/style-brief.*` and extract keywords such as palette, density, image language, typography, motion, and hierarchy.
...
7. Record the chosen preset IDs in `.decksmith/source/style-brief.*` or `.decksmith/source/theme.json`.
